...
def subblock(x, nb_filter, **kwargs):
    ...
    y = keras.layers.Conv2D(keras.backend.int_shape(x)[-1], (1, 1),
                            **kwargs)(y)
...
def build_model(img_shape, lr, l2, activation='sigmoid'):
    ...
    x = keras.layers.Conv2D(
        64, (9, 9), strides=2, activation='relu', **kwargs)(inp)
    ...
    x = keras.layers.BatchNormalization()(x)
    x = keras.layers.Conv2D(
        128, (1, 1), activation='relu', **kwargs)(x)  # 48x48x128
    for _ in range(4):
    ...
    x = keras.layers.BatchNormalization()(x)
    x = keras.layers.Conv2D(
        256, (1, 1), activation='relu', **kwargs)(x)  # 24x24x256
    for _ in range(4):
        x = subblock(x, 64, **kwargs)
    ...
    x = keras.layers.BatchNormalization()(x)
    x = keras.layers.Conv2D(
        384, (1, 1), activation='relu', **kwargs)(x)  # 12x12x384
    for _ in range(4):
    ...
    x = keras.layers.Reshape((4, branch_model.output_shape[1], 1),
                             name='reshape1')(x)
    ...
    x = keras.layers.Conv2D(
        1, (1, mid), activation='linear', padding='valid')(x)
