...
class AdamWithWeightnorm(keras.optimizers.Adam):
    def get_updates(self, loss, params):
        ...
        if self.initial_decay > 0:
            lr *= (1. / (1. + self.decay * keras.backend.cast(self.iterations, keras.backend.floatx())))

        t = keras.backend.cast(self.iterations + 1, keras.backend.floatx())
        lr_t = lr * keras.backend.sqrt(1. - keras.backend.pow(self.beta_2, t)) / (1. - keras.backend.pow(self.beta_1, t))
        ...
        for p, g, m, v in zip(params, grads, ms, vs):
            ...
            if len(ps)>1:
                ...
                v_g_t = (self.beta_2 * v_g) + (1. - self.beta_2) * keras.backend.square(grad_g)
                new_g_param = g_param - lr_t * m_g_t / (keras.backend.sqrt(v_g_t) + self.epsilon)
                self.updates.append(keras.backend.update(m_g, m_g_t))
                self.updates.append(keras.backend.update(v_g, v_g_t))

                # update V parameters
                m_t = (self.beta_1 * m) + (1. - self.beta_1) * grad_V
                v_t = (self.beta_2 * v) + (1. - self.beta_2) * keras.backend.square(grad_V)
                new_V_param = V - lr_t * m_t / (keras.backend.sqrt(v_t) + self.epsilon)
                ...
                v_t = (self.beta_2 * v) + (1. - self.beta_2) * keras.backend.square(g)
...
def get_weightnorm_params_and_grads(p, g):
    ...
    V_scaler = keras.backend.ones(V_scaler_shape)  # init to ones, so effective parameters don't change
