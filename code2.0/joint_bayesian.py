...
def JointBayesian_Train(trainingset, label, fold="./"):
    ...
    for l in range(500):
        ...
        for i in range(n_class):
            ...
            ep[:, nowp:nowp + nn_class] = cur[i].T + np.sum(
                np.dot(SwG[nn_class], cur[i].T), 1).reshape(n_dim, 1)
            nowp = nowp + nn_class
...
def Verify(A, G, x1, x2):
    ...
    x1.shape = (-1, 1)
    x2.shape = (-1, 1)
    ratio = np.dot(np.dot(np.transpose(x1), A), x1) + np.dot(
        np.dot(np.transpose(x2), A),
        x2) - 2 * np.dot(np.dot(np.transpose(x1), G), x2)
    return float(ratio)
...
def data_pre(data):
    ...
    data = np.divide(
        data,
        np.repeat(np.sum(data, 1), data.shape[1]).reshape(
